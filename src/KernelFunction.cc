...
void SqExpKernel::Grad(Col<REAL> &grad, const Col<REAL>& a, const Col<REAL>& b)
{
  ...
  grad(0) = exp(-dist/(2*param(1)*param(1)));
  grad(1) = dist*param(0)*(1/pow(param(1),3))*exp(-dist/(2*param(1)*param(1)));
}
...
void SqExpKernel::GradX(Col<REAL> &grad, const Col<REAL>& a, const Col<REAL>& b)
{
  ...
  if(dist < 1e-6) {
    ...
    grad(0) = -((a(0) - b(0))*param(0))*exp(-dist/(2*param(1)*param(1)))/
      (2*param(1)*param(1)*dist);
    grad(1) = -((a(1) - b(1))*param(0))*exp(-dist/(2*param(1)*param(1)))/
      (2*param(1)*param(1)*dist);
  }
}
